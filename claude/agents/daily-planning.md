...
完了したら、選択された作業リスト(PR/レビュー項目とNotionタスク)と、手順4で作成したSlack投稿用の下書きを、そのまま呼び出し元セッションに返す。呼び出し元は、その結果を使って`daily-task-planning`のサブワークフロー2(直接実装)を自身のモデルで直接実行すること。
