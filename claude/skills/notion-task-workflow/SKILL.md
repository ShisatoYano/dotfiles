...
4. 各タスクの`見積`を人日換算し、以下の目安で並び替えの一次案を作る(機械的な絶対順位ではなく判断材料として扱う):
...
5. 4の一次案から、見積(人日換算)の累計が稼働予算に収まるところまでを候補としてリストアップする
6. 候補リスト(タスク名・優先度・期限有無・見積・ステータス・Notionページへのリンク)をユーザーに提示し、着手済みタスクの扱いやIssueタスクの優先度など状況に応じた入れ替えを含めて確認・調整を受ける。確定したリストがその日の作業対象になる
