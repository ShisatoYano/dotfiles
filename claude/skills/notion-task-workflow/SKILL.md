...
- 日次まとめの保存にはこのdotfilesの`nb`(ノート管理CLI)を使う。notebookは`work`固定。`nb notebooks`で存在を確認し、無ければ`nb notebooks add work`で作成してから使う。`nba`/`nbsum`と同様、`nb add --filename "<path>.md" --content "<text>"`でその場作成する(`shell/aliases.sh`参照)
...
2. `nb add --filename "daily/<YYYY-MM-DD>.md" --content "<まとめ>"`で`work` notebookに保存する。翌朝の`daily-task-planning` Skillがこれを読んで持ち越しを把握する
