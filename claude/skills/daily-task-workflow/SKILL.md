...
2. 1で決めた順に、グループ単位で以下を行う
   - PR/レビュー項目: リスト上から順に1件ずつ、`pr-workflow` Skillの該当ステップ(自分のPRなら「レビューコメントへの対応支援」→「マージ可否チェック」、アサインされたレビューなら「レビュー着手前の軽量ブリーフィング」)をこのセッション内で実行する。実際の投稿・マージ・レビュー実施そのものはユーザー自身が行うため、支援・下書き作成(自分のPRの場合)またはブリーフィング提示(アサインされたレビューの場合)が終わった時点でそのPRは完了とする
