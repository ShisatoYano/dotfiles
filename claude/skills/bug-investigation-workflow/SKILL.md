...
1. Notionタスクページの内容(現象の説明、`何のため？`/`タスクのゴール`等)を確認し、不具合の現象と期待動作を把握する。事前準備で確認した過去の進捗記録があれば、それを踏まえて重複調査を避ける
2. 可能であれば現象を再現する(再現手順の確認・実行)
3. 関連するコード箇所を特定する。単一の最有力候補に飛びつかない。原因がタイポ等の自明な場合を除き、コード以外の要因(パラメータ・設定・閾値の調整不足、環境・データ差異など)も含めて疑わしい箇所を一通り洗い出す。特に制御・航法・状態推定など複雑なアルゴリズムを扱うコードでは、一見有力に見える箇所があっても、それだけで原因と決め付けない
