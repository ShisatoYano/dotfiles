...
4. 期待結果が数値的なpass/failで判定しづらい場合(システムの挙動を可視化して良し悪しを判断する必要があるなど)、判断基準をできる限り定量指標(追従誤差・収束時間・逸脱量など)に落とし込めないか検討する
5. 4で定量化しきれず可視化による判断が残る場合、判断に注目すべきデータ(誤差・軌跡・状態量など)を洗い出し、それを可視化する手段が対象プロジェクトに既存でないか確認する(可視化スクリプト・rviz設定・ロギングツール等)。既存の手段で可視化できない場合は、可視化方法を検討してユーザーに提案する
...
4. 挙動を可視化して判断する場合は、「テスト対象・期待結果の確認」で洗い出した可視化手段を使って判断基準の画像(軌跡プロット・誤差の時系列グラフ・rvizのスクリーンショット等)を生成し、Claude自身がその画像を読んで、事前に定めた判断基準に照らして評価する。可能であれば、既知良好な過去の実行結果(ベースライン)と重ね合わせて比較し、単発の目視印象だけに頼らない
5. シミュレーション終了時は、起動した全プロセスが確実に終了したことをプロセス一覧等で確認してから次に進む。`pkill -f`等のデフォルトシグナル(SIGTERM)は子プロセスに伝播せず孤児化して残り続けることがあり、それが次回以降のローカルテストの誤動作(名前衝突など)につながるため、対象プロジェクトの正規の終了手順を使う
6. failした場合、思いつきで別の修正を試さない。呼び出し元が`bug-investigation-workflow`ならその場で原因調査に戻り、「なぜその修正で直らなかったか」を整理したうえで、洗い出し済みの候補リストの次点を検証してから再提案する
7. 実行結果(pass/fail、ログ、失敗時の原因)をまとめる
