...
| `nbq [query]` | 全notebook横断でripgrep検索。ヒット行を選択 → nvimで編集 |
| `nbmd [query]` | 同上。選択 → `mdroll --watch`でプレビュー |

どちらも中身は共通(`_nb_pick_note`)で、Enterで何を起動するかだけが違う。

- `nb use`での事前切り替えは不要。全notebookが対象
- 打鍵のたびrgを走らせ直す。一覧は`notebook名/ノート名:行番号:ヒット行`で、プレビューはヒット行付近までスクロールした状態で出る
- **本文に加えてファイル名も検索対象**。ファイル名一致は`:1:`の行として本文ヒットより先に並ぶ(連番ファイル名など、名前が本文に出てこないノートに辿り着けなくなるのを防ぐため)
- **クエリはrgの正規表現**。fzfのあいまいマッチ(飛び飛びの一致)は効かない。`(`などのメタ文字をそのまま打つと意図せずグループ扱いになるので、`\(`でエスケープするか部分語で引く
