...
## notion-check(bukuのURLを定期的にタブで開く/アクティブにする)
bukuで`notion_check`タグを付けたURLを、systemdユーザータイマーが平日10時・13時・16時に確認する。
既に開いていればそのタブをアクティブにするだけ(ウィンドウのフォーカスは奪わない)、開いていなければ新規タブで開く。
電源が入っていなかった回は次回起動時に1回だけ追いつく(`Persistent=true`)。

| コマンド | 動作 |
|---|---|
| `buku --nostdin -u <番号> --tag + notion_check` | 既存ブックマークに対象タグを追加 |
| `~/dotfiles/scripts/notion-check.sh` | 手動実行(動作確認用) |
| `systemctl --user list-timers notion-check.timer` | 次回実行予定を確認 |
| `systemctl --user status notion-check.timer` | 有効化状態を確認 |
