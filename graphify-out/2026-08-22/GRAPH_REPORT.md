...
## Corpus Check
- Corpus is ~25,962 words - fits in a single context window. You may not need a graph.
...
- Token cost: 208,579 input · 0 output
...
3. `M.setup()` - 8 edges
4. `dotfiles Repository` - 8 edges
5. `Write-Approval Gate Rule` - 8 edges
6. `Global CLAUDE.md Instructions` - 7 edges
7. `Bug Investigation Workflow Skill` - 7 edges
8. `Dev Workflow Shared Skill` - 7 edges
9. `Notion Task Workflow Skill` - 7 edges
10. `graphify Extraction Subagent Prompt Spec` - 7 edges
...
- `Hook vs Skill Enforcement Split` --semantically_similar_to--> `graphify Post-Commit Auto-Rebuild Hook`  [INFERRED] [semantically similar]
  claude/rules/write-approval.md → claude/skills/graphify/references/hooks.md
- `Vimium h/l = browser back/forward` --references--> `map h goBack / map l goForward`  [EXTRACTED]
  docs/vimium-cheatsheet.md → vimium/custom-key-mappings.txt
...
## Knowledge Gaps
- **43 isolated node(s):** `statusline-command.sh script`, `ai-commit-msg.sh script`, `NVM_DIR`, `ai-squash-commit.sh script`, `setup.sh script` (+38 more)
...
- **What connects `statusline-command.sh script`, `ai-commit-msg.sh script`, `NVM_DIR` to the rest of the system?**
